...
DockerController.prototype.composeUp = function composeUp(p) {
  var file = p.file;
  var name = p.name;
  console.log(`[DOCKER] docker-compose -f${file} -p${name} up -d`);
...
DockerController.prototype.ps = function ps() {
  return this._runDockerColumns(['ps', '--no-trunc']);
};
 ...
 * @return {Q} promise with the result data from outputParser.
 */
DockerController.prototype.__runCmd = function __runCmd(cmd, args, outputParser) {
