...
function CachedEmitter(callbackMap) {
  ...
  if (!callbackMap) {
    throw new Exception('must supply a callback map, ' +
      'otherwise this is just an expensive emitter');
  }
...
CachedEmitter.prototype._updateCache = function(eventId, values) {
  ...
  if (!!this._callbackMap.updateCache) {
    return this._callbackMap.updateCache(
      this._cached[eventId], eventId, values);
  }
