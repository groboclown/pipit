...
module.exports.getByName = function getByName(name) {
  var lambdaVersions = lambdaDepot[name];
  if (!lambdaVersions || lambdaVersions.length <= 0) {
    return null;
  }
  if (lambdaVersions.length != 1) {
    throw new Error(`Too many versions for lambda named ${name}; which one?!?`);
  }
  return lambdaVersions[0];
};
