...
var injections = {
    ...
    invokeBeforeFor: function(serviceName, apiName, req) {
        ...
        for (var i = 0; i < inj.length; i++) {
            res = inj[i].callBefore(serviceName, apiName, req);
    ...
    invokeAfterFor: function(serviceName, apiName, req, response) {
        ...
        for (var i = 0; i < inj.length; i++) {
            res = inj[i].callAfter(serviceName, apiName, req, response);
            if (res !== null && res !== undefined) {
    ...
    monitorDirectory: function(dirname) {
        if (!! dirsWatched[dirname]) {
            return;
        }
        console.log("[Injections] Monitoring injection directory " + dirname);
        var dirWatcher = jsonWatch.createJsonDirEmitter(dirname, {});
        dirWatcher
            .on('add', function(path) {
                injections.registerJsonFile(path);
            })
            .on('change', function(path) {
                injections.registerJsonFile(path);
            })
            .on('unlink', function(path) {
                injections.deregisterJsonFile(path);
            });
        dirWatcher.start();
        dirsWatched[dirname] = dirWatcher;
    },

    stopMonitoringDirectory: function(dirname) {
        if (!! dirsWatched[dirname]) {
    ...
    registerJsonFile: function(filename) {
        injections.deregisterJsonFile(filename);
    ...
    deregisterJsonFile: function(filename) {
...
InjectionStore.prototype.callBefore = function(serviceName, apiName, req) {
    if (!! this.func) {
        return this.func(serviceName, apiName, req);
    }
    return null;
};
InjectionStore.prototype.callAfter = function(serviceName, apiName, req, resp) {
    if (!! this.func) {
        return this.func(serviceName, apiName, req, resp);
    }
    return null;
};
...
if (!! process.env.INJECTION_PATH) {
    injections.monitorDirectory(process.env.INJECTION_PATH);
}
