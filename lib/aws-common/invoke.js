...
module.exports = function(actionMap, aws) {
  ...
  console.log(': aws action ' + actionName);
  ...
  var actionResponseHandler = function(responseList) {
    if (!!responseList) {
      ...
      responseList = injections.invokeAfterFor(
        aws.serviceName, action.Action, aws, responseList);
    }
    return responseList;
  };

  console.log(':: Running action ' + aws.action);
  var response = injections.invokeBeforeFor(
    aws.serviceName, action.Action, aws);
  if (!!response) {
