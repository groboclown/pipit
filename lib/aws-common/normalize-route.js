    ...
    for (var p in actionMap) {
        if (actionMap.hasOwnProperty(p)) {
            ...
            if (typeof action === 'function') {
                ret[p] = module.exports.asRestPath(action);
 ...
 */
module.exports.normalizeVersion = function NormalizeVersion(version, actionMapList) {
    ...
    for (var i = 0; i < actionMapList.length; i++) {
        actionMap = NormalizeRoute(actionMapList[i]);
        for (var p in actionMap) {
            if (actionMap.hasOwnProperty(p)) {
                ...
                if (actionMap[p].UrlPath && actionMap[p].UrlPath.startsWith(versionPrefix)) {
                    ret[p] = {
                        UrlPath: actionMap[p].UrlPath.substr(versionPrefix.length),
 ...
 * (method, path, func).
 */
module.exports.normalizeAction = function() {
    var args = parseRestArguments(arguments.length, arguments[0], arguments[1], arguments[3]);
    var ret = {
        UrlPath: args[1],
        Method: args[0],
        defer: function() {
