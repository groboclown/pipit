...
module.exports.createGenArnFunction = function createGenArnFunction(aws) {
  return function() {
    var args = [aws];
    for (var i = 0; i < arguments.length; i++) {
      args.push(arguments[i]);
    }
    return module.exports.genArn.apply(null, args);
  };
};
...
module.exports.pageResults = function pageResults(vals) {
  ...
  for (var i = 0; (
    ...
    retList.push(resultList[i + pageStart]);
  }
  // ` console.log(`[PAGE] returning ${retList.length} items, starting at ${pageStart}, list size ${resultList.length}`);
  var nextPage = pageStart + retList.length;
  if (nextPage >= resultList.length) {
    nextPage = null;
  } else {
    nextPage = '' + nextPage;
  }
  var ret = { nextPageToken: nextPage };
  ret[key] = retList;
  return ret;
};
