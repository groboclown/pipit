...
module.exports.GetQueueUrl = function(aws) {
  ...
  var accountOwner = psplit(
    aws.params, 'QueueOwnerAWSAccountId', aws.accessKey);
...
module.exports.CreateQueue = function(aws) {
  ...
  if (queueStore.has(aws.accessKey, name)) {
    return [400, 'Sender', 'QueueAlreadyExists',
      'Queue already exists ' + aws.accessKey + '/' + name,
    ];
  }
...
module.exports.SetQueueAttributes = function(aws) {
  ...
  if (!!queue) {
    ...
    return [400, 'Sender',
      ...
      'Unknown attribute name ' + invalid,
    ];
  }
  return [400, 'Sender',
    'AWS.SimpleQueueService.NonExistentQueue',
...
module.exports.ChangeMessageVisibility = function(aws) {
  ...
  if (visibilityTimeout === null || visibilityTimeout === undefined ||
    ...
    return [400, 'Sender',
      ...
      'Invalid visibility timeout ' + visibilityTimeout,
    ];
  }
  if (queue.changeMessageVisibilityByReceiptHandle(
      receiptHandle, visibilityTimeout)) {
    return [200, {}];
  }
  return [400, 'Sender',
    'AWS.SimpleQueueService.NonExistentMessage',
    'Message does not exist ' + receiptHandle,
  ];
};
...
module.exports.ReceiveMessage = function(aws) {
  ...
  if (!!queue) {
    ...
    return queue.pull(maxMessageCount, waitTimeSeconds, visibilityTimeout)
      .then(function pt1(msgs) {
        return [200, {Message: msgs}];
      })
      .catch(function pc1(err) {
        // Generally a timeout
        return [200, {}];
      });
  }
  ...
  if (!!queue) {
    ...
    if (resultCount > 10) {
      return [400, 'Sender',
        ...
        'More than 10 messages sent',
      ];
    }
    if (totalMessageLength > 262144) {
      return [400, 'Sender',
        'BatchRequestTooLong',
        'Total batch message body size > 256k',
      ];
    }
    ...
    psplit.mapAsArrayForEach(messages, function(m) {
      ...
      if (!isValidBatchId(id)) {
        results.BatchResultErrorEntry.push({
          Id: id,
          SenderFault: 'true', // Note: string, not boolean literal
...
var sendOneMessage = function(queue, aws, messageMap) {
  ...
  psplit.mapAsArrayForEach(messageMap.MessageAttribute, function(m) {
    if (!isValidMessageAttributeName(m.Name)) {
      ...
      return [400, 'Sender',
        'InvalidMessageAttributeContents',
        'Incorrect message attribute name: ' + m.Name,
      ];
    }
    if (!m.Value || !m.Value.DataType) {
      // FIXME check error name
      return [400, 'Sender',
        'InvalidMessageAttributeDataType',
        'Incorrect data type for attribute ' + m.Name,
      ];
    }
